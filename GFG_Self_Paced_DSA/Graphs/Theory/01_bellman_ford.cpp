...
struct Graph {
    ...
    // Graph is represented by an array of edges
    struct Edge *edge ;
};
...
//? Bellman-Ford algorithm. The function that detects negative weight cycle.
void BellmanFord ( struct Graph *graph , int src ) {
    ...
    //* src to any other vertex can have atmost |V| - 1 edges.
    for ( int i = 0 ; i < V ; i++ )
    {
        for ( int j = 0 ; j < E ; j++ )
        {
    ...
    //* distances if graph doesn't contain -ve wt cycle. If we get a shorter path, 
...
//? Driver function to run the above function
int main () 
{
